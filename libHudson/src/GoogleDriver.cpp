...
*
* $Id: GoogleDriver.cpp,v1.1 2013-07-28  mreid Exp $
*
...
Series::GoogleDriver::GoogleDriver(void):
  m_linenum(0)
{
}
...
bool Series::GoogleDriver::open(const std::string& filename)
{
...
void Series::GoogleDriver::close(void)
{
  if( m_infile.is_open() )
	  m_infile.close();
...
bool Series::GoogleDriver::next(DayPrice& dp) throw(Series::DriverException)
{
  if( m_infile.eof() )
  	return false;
  ...
  for( TokensRdr::iterator iter = tok.begin(); iter != tok.end(); ++iter, ++i ) {
	  ...
	  switch( i ) {
	    ...
	    case VOLUME:
	      dp.volume = dp.adjclose = atoi(field.c_str());
	      break;
...
bool Series::GoogleDriver::eof(void)
{
  return m_infile.eof();
}
